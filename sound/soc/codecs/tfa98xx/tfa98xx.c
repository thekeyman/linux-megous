...
{	
	...
	if (ret){
		...
		unsigned char *buff = (unsigned char* )kmalloc(TFA2_SPEAKERPARAMETER_LENGTH * sizeof(char), GFP_KERNEL);

		if (buff == NULL) {
			pr_err("can not allocate memory,buff is null\n");
			goto err;
		}
		...
		/* get tcoef data */
		memset(buff, 0x00, sizeof(TFA2_SPEAKERPARAMETER_LENGTH * sizeof(char)));
