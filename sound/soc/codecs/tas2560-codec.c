...
static int tas2560_smartpa_num_get(struct snd_kcontrol *pKcontrol,
				struct snd_ctl_elem_value *pValue)
{
	unsigned int value = 0;
	unsigned int param_id = 0;
	unsigned int length = sizeof(value)/sizeof(unsigned int);
	unsigned int instance = SMARTAMP_INSTANCE_ONE;
	unsigned int index = SMARTAMP_NUMBER_INDEX;
	param_id = (param_id | (index << ALGO_PARAM_INDEX_OFFSET) |
		(length << ALGO_PARAM_LENGTH_OFFSET) |
		(instance << ALGO_PARAM_INSTANCE_OFFSET));
	if(afe_smartamp_algo_ctrl(&value, param_id, TAS_GET_PARAM, sizeof(value), SLAVE1))
	{
		pr_debug("tas2560_smartpa_num_get failed !\n");
		return 0;
	}
	if (value == SMARTAMP_NUM_IS_FOUR)
		pValue->value.integer.value[0] = SMARTAMP_NUM_IS_FOUR;
	else if (value == SMARTAMP_NUM_IS_TWO)
		pValue->value.integer.value[0] = SMARTAMP_NUM_IS_TWO;
	return 0;
}

static int tas2560_smartpa_num_put(struct snd_kcontrol *pKcontrol,
				struct snd_ctl_elem_value *pValue)
{
	unsigned int value = 0;
	unsigned int param_id = 0;
	...
	if (pValue->value.integer.value[0] == SMARTAMP_NUM_IS_FOUR)
		value = SMARTAMP_NUM_IS_FOUR;
	else if (pValue->value.integer.value[0] == SMARTAMP_NUM_IS_TWO)
		value = SMARTAMP_NUM_IS_TWO;

	param_id = (param_id | (index << ALGO_PARAM_INDEX_OFFSET) |
		(length << ALGO_PARAM_LENGTH_OFFSET) |
		(instance << ALGO_PARAM_INSTANCE_OFFSET));
	if(afe_smartamp_algo_ctrl(&value, param_id, TAS_SET_PARAM, sizeof(value), SLAVE1))
		pr_debug("tas2560_smartpa_num_put failed !\n");
...
static const char *tas2560_smartpa_num_text[] = {"Num_NONE", "Num_1", "Num_2", "Num_3", "Num_4"};
static const struct soc_enum  tas2560_smartpa_num_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tas2560_smartpa_num_text),
		tas2560_smartpa_num_text),
};
...
static const struct snd_kcontrol_new tas2560_snd_controls[] = {
	...
	SOC_ENUM_EXT("TAS2560 3 Enable", tas2560_enable_enum[0],
			tas2560_enable_get, tas2560_enable_put),
	SOC_ENUM_EXT("TAS2560 SmartPA Num", tas2560_smartpa_num_enum[0],
			tas2560_smartpa_num_get, tas2560_smartpa_num_put)
};
