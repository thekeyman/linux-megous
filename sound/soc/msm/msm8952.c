...
#define AW87318_MODE 4
static int Spkr_Pa_Ctl_GPIO=0;
static int Spkr_Pa_Status = 0;
static struct delayed_work Ext_Spkr_Amp_En;
...
static char const *mi2s_rx_sample_rate_text[] = {"KHZ_48",
					"KHZ_96", "KHZ_192"};
static const char *const spkr_switch_text[] = {"DISABLE", "ENABLE"};
...
static uint32_t get_mi2s_bits_per_sample(int bits_format)
{
	...
	case SNDRV_PCM_FORMAT_S24_LE:
		bits = 32;	/* 32 bits format */
		break;
	case SNDRV_PCM_FORMAT_S16_LE:
		bits = 16;	/* 16 bits format */
		break;
	default:
		bits = 32;	/* 32 bits format */
		break;
...
static uint32_t get_mi2s_rx_clk_val_custom(int port_id)
{
	...
	case AFE_PORT_ID_QUINARY_MI2S_RX:
		clk_val = mi2s_rx_sample_rate *
				get_mi2s_bits_per_sample(quin_mi2s_rx_bit_format) * 2;
		break;
	case AFE_PORT_ID_QUATERNARY_MI2S_RX:
		clk_val = mi2s_rx_sample_rate *
				get_mi2s_bits_per_sample(quat_mi2s_rx_bit_format) * 2;
		break;
	default:
...
static uint32_t get_mi2s_tx_clk_val_custom(int port_id)
{
	...
	case AFE_PORT_ID_QUINARY_MI2S_TX:
		clk_val = SAMPLING_RATE_48KHZ *
				get_mi2s_bits_per_sample(quin_mi2s_tx_bit_format) * 2;
		break;
	case AFE_PORT_ID_QUATERNARY_MI2S_TX:
		clk_val = SAMPLING_RATE_48KHZ *
				get_mi2s_bits_per_sample(quat_mi2s_tx_bit_format) * 2;
		break;
	default:
...
void ext_spkr_delay_enable(struct work_struct *work)
{
        int i;
	 /* Open  external audio PA device */
	for(i = 0; i < AW87318_MODE; i++) {
           gpio_direction_output(Spkr_Pa_Ctl_GPIO, false);
           gpio_direction_output(Spkr_Pa_Ctl_GPIO, true);
           pr_debug("%s:cnt = %d\n",__func__,i);
	}
...
static void ext_spkr_amp_ctl(bool enable)
{
       int i = 0;
       /* Open or close external audio PA device */
	if(enable){
	      for(i = 0; i < AW87318_MODE; i++) {
                  gpio_direction_output(Spkr_Pa_Ctl_GPIO, false);
                  gpio_direction_output(Spkr_Pa_Ctl_GPIO, true);
...
static int ext_spkr_amp_get(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
       pr_debug("%s: enter \n", __func__);
	ucontrol->value.integer.value[0]=Spkr_Pa_Status;
	return 0;
}
static int ext_spkr_amp_put(struct snd_kcontrol *kcontrol,struct snd_ctl_elem_value *ucontrol)
{
        int state = 0;
	state = ucontrol->value.integer.value[0];
	Spkr_Pa_Status = state;
        pr_debug("%s: set state =%d \n", __func__,state);
        switch(state){
		case 0:
			ext_spkr_amp_ctl(0);
			break;
		case 1:
			schedule_delayed_work(&Ext_Spkr_Amp_En, msecs_to_jiffies(100));
			break;
		default:
			Spkr_Pa_Status=0;
			pr_err("%s: Unexpected input value\n", __func__);
...
static const struct soc_enum msm_snd_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(spkr_switch_text),
				spkr_switch_text),
};
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Spkr_Amp", msm_snd_enum[7],
			ext_spkr_amp_get, ext_spkr_amp_put),
};
...
parse_mclk_freq:
	...
	Spkr_Pa_Ctl_GPIO= of_get_named_gpio(pdev->dev.of_node, "hw,ext_spkr_amp_ctl",0);
	if (Spkr_Pa_Ctl_GPIO < 0) {
		dev_err(&pdev->dev,
			"%s: error! Spkr_Pa_GPIO is :%d\n", __func__, Spkr_Pa_Ctl_GPIO);
	}else{
             if (gpio_request_one(Spkr_Pa_Ctl_GPIO,GPIOF_DIR_OUT ,"spk_enable")) {
                     pr_err("%s: request Spkr_Pa_GPIO  fail!\n",__func__);
		}
	}
       pr_debug("%s:  request Spkr_Pa_GPIO is %d!\n",__func__, Spkr_Pa_Ctl_GPIO);
	...
	INIT_DELAYED_WORK(&Ext_Spkr_Amp_En, ext_spkr_delay_enable);
