...
static bool  disable_voice_lcm_mode = true;
static ssize_t audio_voice_lcm_mode_show(struct device_driver *driver, char *buf)
{
	if (NULL == buf) {
		pr_err("%s: buf is null", __func__);
		return 0;
	}
	return scnprintf(buf, PAGE_SIZE, "%d\n", disable_voice_lcm_mode);
}
DRIVER_ATTR(voice_lvm_mode, 0444, audio_voice_lcm_mode_show, NULL);
