...
static void alarmtimer_shutdown(struct platform_device *dev)
{
        ...
        /* if not in charger mode and cur_time < (60s + alarm_time)
         * the alarm should turn off
         * */
        if (!boot_mode && (ktime_to_ms(min) < 60 * MSEC_PER_SEC)) {
            printk("[alarmtimer] shutdown alarm interval is too short, less than 60s! %s %d\n", __FUNCTION__, __LINE__);
            return;
        }
        /* Setup an rtc timer to fire that far in the future */
        rtc_timer_cancel(rtc, &rtctimer);
        rtc_read_time(rtc, &tm);
        now = rtc_tm_to_ktime(tm);
        /* when not in charger mode,
         * make the alarm trigger ahead of the alarm time, because the power on spend about 60s */
        if (!boot_mode)
            min = ktime_sub(min, ktime_set(60, 0));
