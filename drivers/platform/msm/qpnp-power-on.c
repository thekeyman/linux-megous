...
static irqreturn_t qpnp_kpdpwr_bark_irq(int irq, void *_pon)
{
	...
	extern unsigned int debug_detected_long_key;
	extern struct task_struct *debug_kpdpwr_tk;
	...
	debug_detected_long_key = 0x55aacc33;
	mb();
	wake_up_process(debug_kpdpwr_tk);
	...
	for(i = 0; i < LDO_NUM; i++){
		...
		if((i >= LDO_OCP_NO4 && i <= LDO_OCP_NO18) || (i == LDO_OCP_NO22)){
			if((LDO_OCP_MASK & ldo_status2[i])){
				...
				// LDO dmd info was useless,deleted it.
				}
			}
		}
	}
...
unsigned int debug_detected_long_key = 0;
struct task_struct *debug_kpdpwr_tk = NULL;

static void lp_get_current_time(char *buf, unsigned int buf_len) 
{
	struct timeval tv;
	struct rtc_time tm;
...
static int debug_detect_long_kpdpwr(void *_data)
{
	...
	while (!kthread_should_stop()) {
		if (debug_detected_long_key != 0x55aacc33) {
			set_task_state(current, TASK_UNINTERRUPTIBLE);
			schedule();
			continue;
		}
		debug_detected_long_key = 0;
		do_gettimeofday(&tv);
		lp_get_current_time(time_buf, sizeof(time_buf));
		snprintf(file_name, sizeof(file_name), "/data/log/input-log/long_press_%lu.txt", (unsigned long)tv.tv_sec);
		...
		if (!IS_ERR(fp)) {
			...
			pr_err("long press, can not creat %s\n", file_name);
		}
		fp = NULL;
	}
	return 0;
...
{
	debug_kpdpwr_tk = kthread_run(debug_detect_long_kpdpwr, NULL, "kzwykpdpwr");
	if (!IS_ERR(debug_kpdpwr_tk)) {
		struct sched_param param;
		param.sched_priority = 1;
		sched_setscheduler(debug_kpdpwr_tk, SCHED_FIFO, &param);
		pr_info("long_press_kpdpwr_task running..\n");
	}
