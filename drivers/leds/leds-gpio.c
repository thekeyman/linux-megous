...
static struct gpio_led_data *led_dat_red = NULL;
static struct gpio_led_data *led_dat_green = NULL;
	...
	enum led_brightness value)
{
	...
	if (led_dat_red && led_dat_green) {
		led_dat->value = value;
		if ((led_dat_red->value != LED_OFF) && (led_dat_green == led_dat))
			value = LED_OFF;
		if (led_dat_red == led_dat) {
			if ((led_dat_green->value != LED_OFF) && ((led_dat_red->value == LED_OFF)))
				gpio_set_value(led_dat_green->gpio, 1);
			else
				gpio_set_value(led_dat_green->gpio, 0);
		}
	}
	...
	int (*blink_set)(unsigned, int, unsigned long *, unsigned long *))
{
	...
	if (!strncmp(template->name, "red", strlen("red"))) {
		led_dat_red = led_dat;
		led_dat_red->value = led_dat->cdev.brightness;
	}
	if (!strncmp(template->name, "green", strlen("green"))) {
		led_dat_green = led_dat;
		led_dat_green->value = led_dat->cdev.brightness;
	}
