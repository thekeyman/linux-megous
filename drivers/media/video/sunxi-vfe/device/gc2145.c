...
//	printk("chr wsize.width = [%d], wsize.height = [%d]\n", wsize->width, wsize->height);		
	...
	if (ret)
		return ret;
	vfe_dev_dbg("---------00---------info->capture_mode=%d\n",info->capture_mode);
...
//        vfe_dev_err("sensor_set_capture_exposure err !\n");
    //}
	  ...
	  vfe_dev_dbg("------22------info->flash_mode=%d\n",info->flash_mode);
    #ifdef _FLASH_FUNC_
    if(info->flash_mode!=V4L2_FLASH_LED_MODE_NONE)
    {
		vfe_dev_dbg("------to_flash=%d\n",to_flash);
      if(to_flash==1)
      {
        printk("open flash when capture\n");
