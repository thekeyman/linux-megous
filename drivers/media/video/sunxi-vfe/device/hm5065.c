...
//		  struct csi_dev *dev=(struct csi_dev *)dev_get_drvdata(sd->v4l2_dev->dev); 
 ...
 //  struct regval_list test_reg[] = { 
//		{0x0049,0x33}, 
//   	};
   struct regval_list test1_reg[] = { 
		{0x0010,0x0}, 
   	};
   struct regval_list test2_reg[] = { 
		{0x0010,0x01}, 
  }; 
  //reset sensor MCU 
  ...
  msleep(100); 
   ret = sensor_write_array(sd, test1_reg ,ARRAY_SIZE(test1_reg)); 
  if(ret < 0) { 
  ...
  if(ret < 0) { 
   ...
   ret = sensor_write_array(sd, test2_reg ,ARRAY_SIZE(test2_reg)); 
  if(ret < 0) { 
	vfe_dev_err("start af firmware error\n"); 
	return ret; 
  } 
  ...
  msleep(10); 
  //Don't need check the af firmware status at HM5065 
  rdval = 0xff; 
  cnt = 0; 
  recheck_af_fw:		 
...
//	vfe_dev_dbg("sensor_g_single_af\n"); 
  ...
  return V4L2_AUTO_FOCUS_STATUS_IDLE;		 //0528 
  
  rdval = 0xff; 
  
  LOG_ERR_RET(sensor_read(sd, 0x07AE, &rdval)) 
  vfe_dev_print("sensor_g_single_af function, 0x07AE=0x%x\n",rdval); 

  if(rdval == 0x01) 
  { 
	int ret=0; 
	info->focus_status = 0; //0-0528		idle 
	sensor_read(sd, 0x07AE, &rdval); 

		  vfe_dev_dbg("HM5065_Single AF focus ok, idle mode, 0x07AE = 0x%x\n",rdval); 
  ...
  {   
		  info->focus_status = 1;		 //1-0528 
	vfe_dev_print("HM5065_Single AF focus ing, busy mode, 0x07ae = 0x%x\n",rdval); 
	return V4L2_AUTO_FOCUS_STATUS_BUSY; 	   //V4L2_AUTO_FOCUS_STATUS_BUSY				0528 
  } 
...
{ 
  ...
  LOG_ERR_RET(sensor_read(sd, 0x0040, &rdval)); 
  vfe_dev_print("sensor_g_single_af function, 0x0040=0x%x\n",rdval); 


  LOG_ERR_RET(sensor_read(sd, 0x0041, &rdval)); 
  vfe_dev_print("sensor_g_single_af function, 0x0041=0x%x\n",rdval); 
...
//	vfe_dev_dbg("sensor_g_contin_af\n"); 
  ...
  if(rdval == 0x01) 
  { 
	unsigned char rdval2;
	LOG_ERR_RET(sensor_read(sd, 0x02d1, &rdval2)); 
	
	info->focus_status = 0; //idle 
				vfe_dev_dbg("Contin AF focus ok, 0x07ae = 0x%x, 0x02d1= 0x%x\n",rdval, rdval2); 
  ...
  { 
	...
	vfe_dev_dbg("Contin AF focus ing,0x07ae = 0x%x\n",rdval); 
	return V4L2_AUTO_FOCUS_STATUS_BUSY; 
  } 
...
//static int sensor_s_continueous_af(struct v4l2_subdev *sd) 
{
		...
		int ret;
/*		
		 struct regval_list reset1_reg[] = { 
		{0x0010,0x0}, 
   	};
   struct regval_list reset2_reg[] = { 
		{0x0010,0x01}, 
  }; 
  */
  struct sensor_info *info = to_state(sd); 
...
//	if( (info->auto_focus==value) )
 ...
 // if(value==1)
  {
 // ret = sensor_write_array(sd, reset1_reg ,ARRAY_SIZE(reset1_reg)); 
 // if(ret < 0) { 
//	vfe_dev_err("reset af firmware error\n"); 
//	return ret; 
 // } 
 //  msleep(100);
    
  //  LOG_ERR_RET(sensor_write(sd, 0x0751, 0x00))
  	LOG_ERR_RET(sensor_write(sd, 0x070a, 0x01))
    	msleep(100);		 
 // ret = sensor_write_array(sd, reset2_reg ,ARRAY_SIZE(reset2_reg)); 
 // if(ret < 0) { 
//	vfe_dev_err("reset af firmware error\n"); 
//	return ret; 
// } 
...
//		ret = sensor_read(sd, 0x0751, &rdval); 	
//		vfe_dev_print("AF firmware check status complete, 0x0751 = 0x%x\n",rdval); 
//		ret = sensor_read(sd, 0x070a, &rdval); 
//		vfe_dev_print("AF firmware check status complete, 0x070a = 0x%x\n",rdval); 
														...
														struct v4l2_win_coordinate * win_c)
{
  ...
  printk("zzzzzzzzzzz------sensor_s_af_zone\n");
  
  ret = sensor_write(sd, 0x0808, 0x01); 	//0528
  ret = sensor_write(sd, 0x0809, 0x00);
  ret = sensor_write(sd, 0x080A, 0x00);
  ret = sensor_write(sd, 0x080B, 0x00);
  ret = sensor_write(sd, 0x080C, 0x00);
  ret = sensor_write(sd, 0x080D, 0x00);
  ret = sensor_write(sd, 0x080E, 0x00);
  ret = sensor_write(sd, 0x0751, 0x00);
  ret = sensor_write(sd, 0x0714, 0x00);
  usleep_range(5000,6000);
  ...
  vfe_dev_dbg("af zone input x1=%d,y1=%d\n",x1,y1);
  vfe_dev_dbg("af zone input x2=%d,y2=%d\n",x2,y2);
  ...
  //set x center
  ret = sensor_write(sd, 0x0715, xc>>8);		//0528
  ret = sensor_write(sd, 0x0716, xc&0xFF);		//0528
  if (ret < 0)
  {
  ...
  //set y center
  ret = sensor_write(sd, 0x0719, yc>>8);		//0528
  ret = sensor_write(sd, 0x071A, yc&0xFF);		//0528
  if (ret < 0)
  {
	vfe_dev_err("sensor_s_af_zone_yc error!\n");
	return ret;
  }
  ret = sensor_write(sd, 0x0717, 0x01); 	//0528
  ret = sensor_write(sd, 0x0718, 0x40); 	//0528
  ret = sensor_write(sd, 0x071B, 0x01); 	//0528
  ret = sensor_write(sd, 0x071C, 0x40); 	//0528		
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
  ...
  vfe_dev_dbg("======0711 0 sensor_init 0x%x\n",val);
             ...
             struct v4l2_mbus_framefmt *fmt)//linux-3.0
{
	...
	*/		
	...
	else if(info->capture_mode == V4L2_MODE_VIDEO || info->capture_mode == V4L2_MODE_PREVIEW)
    {		
		// if(info->auto_focus==1)
		 {
...
//  case V4L2_CID_AUTO_FOCUS_STOP:
  case V4L2_CID_AUTO_FOCUS_STATUS:
  	printk("V4L2_CID_AUTO_FOCUS_STATUS\n");
