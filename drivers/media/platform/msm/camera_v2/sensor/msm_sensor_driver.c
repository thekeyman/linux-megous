	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (!strcmp(slave_info->sensor_name, "ov13855_sunny")) {
		...
		if (main_module_id != 0x6 || main_module_id == 0xb) {
			pr_err("failed: main_module_id %d, sensor is not %s", main_module_id, slave_info->sensor_name);
			rc = -EINVAL;
			goto free_slave_info;
		}
	} else if (!strcmp(slave_info->sensor_name, "hi553_kingcome")) {
		if (sub_module_id != 2) {
	...
	} else if (!strcmp(slave_info->sensor_name, "s5k5e8_kerr")) {
		if (sub_module_id != 0xc) {
			pr_err("failed: main_module_id %d, sensor is not %s", sub_module_id, slave_info->sensor_name);
			rc = -EINVAL;
			goto free_slave_info;
		}
	} else if (!strcmp(slave_info->sensor_name, "ov5675_yeji")) {
		if (sub_module_id != 0x10) {
			pr_err("failed: main_module_id %d, sensor is not %s", sub_module_id, slave_info->sensor_name);
			rc = -EINVAL;
			goto free_slave_info;
		}
