...
/*support hi553 otp start*/
struct sensor_otp_reg_t {
	uint32_t addr;
	uint32_t data;
};
struct sensor_otp_reg_t hi553_init_otp[] = {
...
static int hi553_get_otp(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int rc = 0;
	uint16_t data[24] = {0};
	uint16_t j;
	...
	for (j = 0; j < sizeof(hi553_init_otp)/sizeof(struct sensor_otp_reg_t); j++) {
		e_ctrl->i2c_client.addr_type = 2;
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), hi553_init_otp[j].addr,hi553_init_otp[j].data, 2);
		if (rc < 0) {
	...
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x003f, 0x10, 1);
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x0a00, 0x01, 1);

	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x10a, (0x501>>8) & 0xff, 1);
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x10b, 0x501 & 0xff, 1);
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0x102, 0x01, 1);

	for (j=0; j < 24; j++)  {
		rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_read(&(e_ctrl->i2c_client), 0x108, &data[j], 1);
		memptr[j] = data[j];
		CDBG("%s: read data=%x, j=%d, memptr=%x\n", __func__, data[j], j, memptr[j]);
	}
...
/*support hi553 otp end*/

/*support gc5005 otp start*/
static int gc5005_get_otp(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int rc = 0;
	uint16_t data[24] = {0};
	uint16_t j;
	...
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xf9, 0xaa, 1);
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xfa, 0x84, 1);
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xfc, 0x8a, 1);

	//enable otp and select page1
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xfa, 0x94, 1);//bit4 otp clk enable
	rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xd4, 0x84, 1);//bit7 otp enable, bit2 page0/1

	for (j=0; j < 24; j++)  {
		rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xfe, 0x00, 1);
		rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xd5, j, 1);
		rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_write(&(e_ctrl->i2c_client), 0xf3, 0x20, 1);
		rc += e_ctrl->i2c_client.i2c_func_tbl->i2c_read(&(e_ctrl->i2c_client), 0xd7, &data[j], 1);
		memptr[j] = data[j];
		CDBG("%s: read data=%x, j=%d, memptr=%x\n", __func__, data[j], j, memptr[j]);
	}
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	if (e_ctrl->userspace_probe == 0) {
		...
		printk("%s line:%d, eeprom-name %s\n", __func__, __LINE__, eb_info->eeprom_name);
		if (!strcmp(eb_info->eeprom_name, "hi553_kingcome")) {
			rc = hi553_get_otp(e_ctrl);
			if (rc < 0) {
				pr_err("%s hi553_get_otp failed\n", __func__);
				goto power_down;
			}
		} else {
			rc = read_eeprom_memory(e_ctrl, &e_ctrl->cal_data);
		...
		if (!strcmp(eb_info->eeprom_name, "ov13855_sunny")) {
			...
			if (module_id == 1) {
				printk("match id for ov13855_sunny success\n");
				main_module_id = module_id;
			} else {
				pr_err("%s match id for ov13855_sunny failed\n", __func__);
				goto power_down;
			}
		} else if (!strcmp(eb_info->eeprom_name, "hi553_kingcome")) {
			...
			printk("match id for hi553_kingcome module_id=%d\n", module_id);
			if (module_id == 2) {
				printk("match id for hi553_kingcome success\n");
				sub_module_id = module_id;
			} else {
				pr_err("%s match id for hi553_kingcome failed\n", __func__);
				goto power_down;
			}
		} else if (!strcmp(eb_info->eeprom_name, "s5k3l8_ofilm")) {
			printk("match id for s5k3l8_ofilm\n");
			if (e_ctrl->cal_data.mapdata[0] == 0x55) {
				module_id = e_ctrl->cal_data.mapdata[1] & 0x1f;
			} else {
				module_id = -1;
			...
			if (module_id == 7) {
				printk("match id for s5k3l8_ofilm success\n");
				main_module_id = module_id;
			} else {
				pr_err("%s match id for s5k3l8_ofilm failed\n", __func__);
				goto power_down;
			}
		} else if (!strcmp(eb_info->eeprom_name, "s5k5e8_kerr")) {
			if (e_ctrl->cal_data.mapdata[16] == 0x55) {
				module_id = e_ctrl->cal_data.mapdata[17] & 0x1f;
			} else if (e_ctrl->cal_data.mapdata[8] == 0x55) {
				module_id = e_ctrl->cal_data.mapdata[9] & 0x1f;
			...
			printk("match id for s5k5e8_kerr module_id=%d\n", module_id);
			if (module_id == 0xc) {
				printk("match id for s5k5e8_kerr success\n");
				sub_module_id = module_id;
			} else {
				pr_err("%s match id for s5k5e8_kerr failed\n", __func__);
				goto power_down;
			}
			...
			if (module_id == 0x6 || module_id == 0xb) {
				printk("match id for ov13855_qtech success\n");
				main_module_id = module_id;
			} else {
				pr_err("%s match id for ov13855_qtech failed\n", __func__);
				goto power_down;
			}
		} else if (!strcmp(eb_info->eeprom_name, "ov5675_yeji")) {
			if (e_ctrl->cal_data.mapdata[16] == 0x55) {
				module_id = e_ctrl->cal_data.mapdata[17] & 0x1f;
			} else if (e_ctrl->cal_data.mapdata[8] == 0x55) {
				module_id = e_ctrl->cal_data.mapdata[9] & 0x1f;
			...
			printk("match id for ov5675_yeji module_id=%d\n", module_id);
			if (module_id == 0x10) {
				printk("match id for ov5675_yeji success\n");
				sub_module_id = module_id;
			} else {
				pr_err("%s match id for ov5675_yeji failed\n", __func__);
				goto power_down;
			}
		} else {
		...
		printk("%s eeprom module id: main_module_id=%d  sub_module_id=%d\n", __func__, main_module_id, sub_module_id);
