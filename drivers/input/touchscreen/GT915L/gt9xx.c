...
#define GTP_READ_LOCKDOWN_INFO_ADDR  0xC8A4 //: fw-8001           0x81A0 :fw-3007
static u8 lockdown_info[GTP_LOCKDOWN_SIZE];
static u8 TP_Maker,LCD_Maker,Panel_Ink;
//extern u8 tp_color;
u8 tp_color;//Modify@wangdongbo.wt_20161123_define out of this file
static u16 IC_Version;
static int gtp_read_lockdown_info(void);
int gtp_hardwareinfo_set(void);
#endif

static const char *goodix_ts_name = "goodix-tp";
...
//add by hubing1.wt 2016.09.30 for unified macro control
...
#endif
//-bug 217588,Add gtp lockdown node,@cm-hezhengliang.wt_20160918
...
//add by hubing.wt 2016.09.18 for gesture_wake_up
...
//begin_add by hubing.wt 2016.09.18 for gesture_wake_up
    ts->input_dev->event= fts_input_event;
//end_add by hubing.wt 2016.09.18 for gesture_wake_up
...
void parse_cmdline_for_gt915(void)
{
	...
	cmdline_tp = strstr(saved_command_line, "androidboot.mode=");		
 	if(cmdline_tp != NULL)
 	{
		...
		ret = strncmp(temp, "ffbm", strlen("ffbm"));//ffbm -02 or  ffbm -01
		if(ret==0)
		{
			/* factory mode*/
			gtp_ics_slot_report= 1;
			return;
		}		
 	}
...
#if GTP_AUTO_UPDATE
//Modify, cm-hezhengliang.wt_20160818_for_factory_test_current
...
static int gtp_read_lockdown_info(void)
{
	...
	//fix by hubing1.wt for fix lockdown_info 2016.10.09
...
//guojunbo.wt modify for suspend be called sometimes satrt 
if (1 == ts->gtp_is_suspend)
{
	return;
}
//guojunbo.wt modify for suspend be called sometimes end
...
#endif
 GTP_INFO("gtp: 11 driver installing...");
    ret = i2c_add_driver(&goodix_ts_driver);
GTP_INFO("gtp: 22 driver installing..."); 
   return ret; 
}
