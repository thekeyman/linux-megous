...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    {
        printk("Update GT915S to GT9158 directly!\n");
        return SUCCESS;
    }
    ...
    printk("Firmware length:%d(%dK)\n", update_msg.fw_total_len, update_msg.fw_total_len/1024);
    
    if (update_msg.force_update != 0xBE)
    {
        printk("FW chksum error,need enter update.\n");
        return SUCCESS;
    }
    ...
    {
        printk("Illegal IC pid, need enter update\n");
        return SUCCESS;
    }
    else
    {
        for (i = 0; i < 3; i++)
        {
            ...
            {
                printk("Illegal IC pid, out of bound, need enter update");
                return SUCCESS;
            }
        }
    }
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        printk("wdb: ic fw:%d,  file fw:%d \n",update_msg.ic_fw_msg.vid, fw_head->vid);
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    //u8 GTP_FW_SENSOR_ID = 0;

    got_file_flag = 0x00;
    if (path)
    {
        printk("Update File path:%s, %ld", path, strlen(path));
...
/*
	printk("wdb:start read sensor id !!!\n");
	 ret = gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID, &GTP_FW_SENSOR_ID, 1);
	  if (SUCCESS == ret)
	  {
		if (GTP_FW_SENSOR_ID >= 0x09)
		{
			...
			//Modify@wangdongbo.wt_20161123_force_upgrade_firmware
			//return FAIL;
		}
	}

	printk("wdb: GTP_FW_SENSOR_ID:%d \n", GTP_FW_SENSOR_ID);
...
s32 gup_update_proc(void *dir)
{
    ...
    printk("%s:%d:begin to judge update now...\n",__func__,__LINE__);

     printk("[update_proc] STEP 3 \n");
...
//begin-add by hubing1.wt 2016.09.30 for unified macro control
#if  WT_ADD_CTP_INFO
gtp_hardwareinfo_set();
#endif
//end-add by hubing1.wt 2016.09.30 for unified macro control
