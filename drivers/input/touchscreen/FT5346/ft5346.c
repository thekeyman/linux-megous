...
u8 state;
    ...
    dev_err(&data->client->dev, "tp:int\n");
...
static int fts_ctpm_fw_upgrade_with_i_file(struct ft5x06_ts_data *data)
{
   ...
   fts_updateinfo_curr.CHIP_ID = ic_type;
    ...
    printk("Vendor ID:0x%02X, TP FW:0x%02X, IC TYPE:%d", vendor_id,uc_tp_fm_ver,ic_type);
        ...
        && (pbt_buf[fw_len - 3] ^ pbt_buf[fw_len - 4]) == 0xFF)
    {
        ...
        CTP_DEBUG("[FTS] is_ic_update_crash = %d.\n", is_ic_update_crash);

        if((uc_tp_fm_ver < uc_host_fm_ver)||(is_ic_update_crash==1))
        {
            CTP_DEBUG("[FTS] : upgrade_start \n");
