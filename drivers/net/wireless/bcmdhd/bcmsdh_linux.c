...
int bcmsdh_register_oob_intr(void * dhdp)
{
  ...
  if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
    SDLX_MSG(("has no wl_host_wake_invert\n"));
  } else {
    host_wake_invert = val.val;
  }
  
  if (!host_wake_invert)
	  error = request_irq(sdhcinfo->oob_irq, wlan_oob_irq, IRQF_TRIGGER_HIGH, "bcmsdh", NULL);
  else
    error = request_irq(sdhcinfo->oob_irq, wlan_oob_irq, IRQF_TRIGGER_LOW, "bcmsdh", NULL);
