...
static int therm_chg = 0;
static int therm_cpu = 0;
static int therm_pa = 0;
...
static int set_pa_temp(const char *val, struct kernel_param *kp)
{
	int ret;
...
#ifdef CONFIG_HW_TEMP_TEST
	if (therm_pa != 0)
		results.physical = therm_pa;
#endif
	return snprintf(buf, BUF_MAX_LENGTH, "%d", (int)results.physical);
}
module_param_call(therm_pa, set_pa_temp, get_pa_temp, &therm_pa, 0644);

#ifdef CONFIG_HW_TEMP_TEST
static int set_cpu_temp(const char *val, struct kernel_param *kp)
{
	int ret;
...
#ifdef CONFIG_HW_TEMP_TEST
	if (therm_cpu != 0)
		results.physical = therm_cpu;
#endif
	return snprintf(buf, BUF_MAX_LENGTH, "%d", (int)results.physical);
}
module_param_call(therm_cpu, set_cpu_temp, get_cpu_temp, &therm_cpu, 0644);

#ifdef CONFIG_HW_TEMP_TEST
static int set_chg_temp(const char *val, struct kernel_param *kp)
{
	int ret;
...
static int get_chg_temp(char *buf, struct kernel_param *kp)
{
	int rc = 0;
	struct qpnp_vadc_result results;
	if (chg_mpp_number < 0 || qpnp_pmi_vadc == NULL) {
		return 0;
	}
	
	rc = qpnp_vadc_read(qpnp_pmi_vadc, chg_mpp_number, &results);
	if (rc) {
...
#ifdef CONFIG_HW_TEMP_TEST
	if (therm_chg != 0)
		results.physical = therm_chg;
#endif
	return snprintf(buf, BUF_MAX_LENGTH, "%d", (int)results.physical);
}
module_param_call(therm_chg, set_chg_temp, get_chg_temp, &therm_chg, 0644);
