...
int de_enhance_set_mode(unsigned int format, struct disp_enhance_config *config)
{
	...
	unsigned int enhance_mode_to_para[MODE_NUM][FORMAT_NUM][PARA_NUM] =
	{
		/* lcd */
		...
		/* hdmi */
		{//SHARP    //AUTO_CONTRAST  //AUTO_COLOR  //FC_R    //FC_G    //FC_B
			{0x11,    0x10001,     0x21,      1,        1,        1}, /* rgb */
			{0x11,    0x10020,     0x21,      2,        2,        2}, /* yuv */
		},
	};
	...
	{
		__wrn("de_enhance_set_mode : enhance mode excess maximum mode number!! mode = %d, max_mode = %d., format=%d\n", mode, MODE_NUM, format);
		return 1;
	}

	config->info.sharp = enhance_mode_to_para[mode][format][0];
	config->info.auto_contrast = enhance_mode_to_para[mode][format][1];
	config->info.auto_color = enhance_mode_to_para[mode][format][2];
	config->info.fancycolor_red = enhance_mode_to_para[mode][format][3];
	config->info.fancycolor_green = enhance_mode_to_para[mode][format][4];
	config->info.fancycolor_blue = enhance_mode_to_para[mode][format][5];
