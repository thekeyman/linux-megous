...
{	
	...
	pn544_dev->irq_gpio =  GPIOL(6);	
	pn544_dev->ven_gpio  = GPIOD(2);
	pn544_dev->firm_gpio  = GPIOD(3);
...
{
	struct i2c_board_info pn544_info; //硬件信息
	struct i2c_adapter *adapter; //总线驱动
	struct i2c_client *client;
	int ret;
	pr_info("Loading pn544 driver\n");
	ret = i2c_add_driver(&pn544_driver);
	if (ret < 0) {
	    pr_info("add  pn544 driver failed\n");
	}
	//分配初始化注册i2c_client
	//初始化i2c_board_info
	memset(&pn544_info, 0, sizeof(pn544_info));
	//初始化type,这个字段最终会给i2c_client.name
	strlcpy(pn544_info.type,PN544_NAME,I2C_NAME_SIZE);
	//初始化设备地址
	pn544_info.addr = PN544_ADDRESS;

	//获取总线驱动信息
	adapter = i2c_get_adapter(twi_id); //指定总线编号
	//分配初始化注册i2c_client,然后跟i2c_driver
	//进行匹配，如果匹配成功调用probe函数
	client = i2c_new_device(adapter, &pn544_info);
	//释放总线驱动信息
