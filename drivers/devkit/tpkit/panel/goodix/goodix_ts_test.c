...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	GTP_DEBUG("special_raw_data_limit :%u",size);
	...
	if (size > 0 && (size % 4) == 0) {
		...
		for (i = 0; i < nodes; i++) {
			...
			reorder = row * ts_test->test_params.sc_sen_num+ column;
			if (array[reorder * 4]  < MAX_SEN_NUM * MAX_DRV_NUM) {
				...
				ts_test->test_params.deviation_limits[i] =
						array[reorder * 4 + 3];
				GTP_DEBUG("reorder : %u    raw_data_limit : %u   %u   %u",reorder,ts_test->test_params.max_limits[i],ts_test->test_params.min_limits[i] ,ts_test->test_params.deviation_limits[i]);
			} else {
...
GTP_INFO("rawdata sc_sensor is [%d],sc_drv_num[%d] size is [%d]",test_params->sc_sen_num, test_params->sc_drv_num,rawdata->size);
	for (i = 0; i < rawdata->size; i++) {
		GTP_DEBUG("rawdata[%d][%d]:%u ------max_limit:%u -----mix_limit:%u",
				i / test_params->sc_sen_num, i % test_params->sc_sen_num,
				rawdata->data[i], test_params->max_limits[i], test_params->min_limits[i]);
				...
				struct ts_test_params *test_params)
{
	...
	for (i = 0; i < sc_data_num; i++) {
		...
		max_val /= rawdata_val;
		GTP_DEBUG("deviation[%d][%d]:%u --------------",i / cols, i % cols,max_val);
		GTP_DEBUG("test_params->deviation_limits:%u ",test_params->deviation_limits[i]);
...
static int goodix_short_test_prepare(struct goodix_ts_test *ts_test)
{
	...
	//ADC Read Delay
    data[0] = (150 >> 8) & 0xff;
    data[1] = GTP_ADC_READ_DELAY & 0xff;
