	...
	struct goodix_ts_data *goodix_pdata)
{
	...
	dev_data = goodix_pdata->goodix_device_data;
	...
	if (ret) {
		dev_data->ic_type = ONCELL;
		TS_LOG_ERR("Not define device ic_type in Dts\n");
	}
	else {
		g_tskit_ic_type = dev_data->ic_type;
	}
	TS_LOG_INFO("get g_tskit_ic_type = %d.\n", g_tskit_ic_type);

	ret = of_property_read_u32(np, GTP_PROJECTID_ADDR, &goodix_pdata->projectid_addr);
	if (ret) {
		goodix_pdata->projectid_addr = GTP_PROJECTID_DEFAULT_ADDR;
		TS_LOG_ERR("%s:get projectid_addr from dts failed ,use default addr 0x%0x \n", __func__, goodix_pdata->projectid_addr);
	}

	ret = of_property_read_u32(np, GTP_WD_CHECK,
						&dev_data->need_wd_check_status);
	if (ret) {
		GTP_ERROR("need_wd_check_status parmerter not exit use data");
		dev_data->need_wd_check_status = false;
	}

	ret = of_property_read_u32(np, GTP_WD_TIMEOUT,
