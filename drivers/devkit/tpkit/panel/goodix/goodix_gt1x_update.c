...
s32 gup_enter_update_mode(void)
{
    ...
    int reset_gpio = goodix_ts->ts_platform_data->reset_gpio;
    ...
    while(retry++ < 200)
    {
        ...
        if(ret <= 0)
        {
            TS_LOG_ERR("Hold ss51 & dsp I2C error,retry:%d", retry);
            continue;
        }
        ...
        TS_LOG_ERR("Hold ss51 & dsp confirm 0x4180 failed,value:%d", rd_buf[0]);
    }
    if(retry >= 200)
    {
        TS_LOG_ERR("Enter update Hold ss51 failed.");
        return FAIL;
    }
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    TS_LOG_INFO("FILE HARDWARE INFO:%02x%02x%02x%02x\n", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    TS_LOG_INFO("FILE PID:%s\n", fw_head->pid);
    TS_LOG_INFO("FILE VID:%04x\n", fw_head->vid);
    TS_LOG_INFO("IC HARDWARE INFO:%02x%02x%02x%02x\n", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
             update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    TS_LOG_INFO("IC PID:%s\n", update_msg.ic_fw_msg.pid);
    TS_LOG_INFO("IC VID:%04x\n", update_msg.ic_fw_msg.vid);
            ...
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        //The third condition
        if ( ( (goodix_ts->fw_type == GOODIX_AUTO_UPGRADE_FW)
		    && (fw_head->vid > update_msg.ic_fw_msg.vid))
		|| (goodix_ts->fw_type == GOODIX_MANUAL_UPGRADE_FW) )
        {
            TS_LOG_INFO("Need enter update.\n");
            return SUCCESS;
        }
		goodix_ts->fw_version_flag = 1;
        TS_LOG_INFO("Don't meet the third condition.\n");
        TS_LOG_INFO("File VID <= Ic VID, update aborted!\n");
    }
    else
    {
        TS_LOG_ERR("File PID != Ic PID, update aborted!\n");
    }
...
static u8 gup_check_update_file(st_fw_head* fw_head)
{
	...
	if (update_msg.update_type == UPDATE_TYPE_HEADER) {
		...
		/* check upgrade type */
		if(goodix_ts->fw_type)
			fw_name = goodix_ts->manual_fw_name;
		else
			fw_name = goodix_ts->auto_fw_name;

		//ret = request_firmware(&update_msg.fw, GT9XX_FW_NAME,&goodix_ts->pdev->dev);
		ret = request_firmware(&update_msg.fw, fw_name, &goodix_ts->pdev->dev);
		if (ret < 0) {
		...
		if (update_msg.fw->size <
			...
			TS_LOG_ERR("INVALID firmware!");
			return FAIL;
		}
		update_msg.fw_total_len = update_msg.fw->size - FW_HEAD_LENGTH;

		TS_LOG_INFO("Firmware actual size: %d(%dK)",
		...
		TS_LOG_INFO("firmware checksum:%x", fw_checksum&0xFFFF);
		if (fw_checksum&0xFFFF)
		{
			TS_LOG_ERR("Illegal firmware file.");
			return FAIL;
		}
		...
		return SUCCESS;
	        ...
	        if (got_file_flag & CFG_FILE_READY)
	        {
	            ...
	            if(ret <= 0)
	            {
	                TS_LOG_ERR("Update config failed.");
	            }
	        ...
	        if ( !(got_file_flag & BIN_FILE_READY) )
	        {
	            TS_LOG_ERR("No bin file for fw update");
	            return FAIL;
              }
    ...
    if (update_msg.fw_total_len < (FW_HEAD_LENGTH + FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
    {
        ...
        TS_LOG_ERR("INVALID bin file(size: %d), update aborted.", update_msg.fw_total_len);
        return FAIL;
    }

    update_msg.fw_total_len -= FW_HEAD_LENGTH;
    TS_LOG_INFO("Bin firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
    ...
    {
	set_fs(update_msg.old_fs);
        TS_LOG_ERR("Read firmware head in update file error.");
        return FAIL;
    }
    ...
    for(i=0; i<update_msg.fw_total_len; i+=2)
    {
        ...
        {
			set_fs(update_msg.old_fs);
            TS_LOG_ERR("Read firmware file error.");
            return FAIL;
        }
...
s32 gup_update_proc(int type)
{
    ...
    {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_CHECK_FILE_FAIL;
#endif
        TS_LOG_ERR("[update_proc]check update file fail.");
        goto file_fail;
    }
    ...
    {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_GET_IC_FW_MSG_FAIL;
#endif
        TS_LOG_ERR("[update_proc]get ic message fail.");
        goto file_fail;
    }

	TS_LOG_INFO("[gup_update_proc] before gup_enter_update_judge.\n");

    ret = gup_enter_update_judge(&fw_head);
    if(FAIL == ret)
    {
        TS_LOG_ERR("[update_proc]Check *.bin file fail.\n");
        goto file_fail;
    }
    ...
    {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_ENTER_UPDATE_MODE_FAIL;
#endif
    ...
    while(retry++ < 5)
    {
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_DSP_ISP_FAIL;
#endif
	 ...
	 ret = gup_burn_fw_gwake();
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_APP_ISP_FAIL;
#endif
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_SS51_ISP_FAIL;
#endif
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_DSP_FW_FAIL;
#endif
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_FW_BOOT_FAIL;
#endif
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_FW_BOOT_ISP_FAIL;
#endif
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_FW_LINK_FAIL;
#endif
        ...
        {
#if defined (CONFIG_HUAWEI_DSM)
	g_goodix_dev_data->ts_platform_data->dsm_info.constraints_UPDATE_status =  GOODIX_BURN_FW_FINISH_FAIL;
#endif
    ...
    if (retry >= 5)
    {
        TS_LOG_ERR("%s: retry %d timeout,UPDATE FAIL.", __func__, retry);
        update_ret = FAIL;
    }
    else
    {
