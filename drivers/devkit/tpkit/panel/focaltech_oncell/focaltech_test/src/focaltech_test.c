...
#define FTS_INI_FILE_PATH "/product/etc/firmware/"
...
static int fts_test_i2c_read(unsigned char *writebuf, int writelen, unsigned char *readbuf, int readlen)
{
   ...
   //iret = ft5x06_i2c_read(fts_i2c_client, writebuf, writelen, readbuf, readlen);
    if (writelen > 0)
    {
        struct i2c_msg msgs[] =
        {
...
static int fts_test_i2c_write(unsigned char *writebuf, int writelen)
{
    ...
    //Modify the i2c_read function that is used in this project
   // iret =ft5x06_i2c_write(fts_i2c_client, writebuf, writelen);
...
///////////////////////////////////
extern int focal_write(u8 *values, u16 values_size);
extern int focal_read(u8 *addrs, u16 addr_size, u8 *values, u16 values_size);
static int fts_test_entry(char *ini_file_name, char *bufdest, ssize_t* pinumread)
{
    ...
    init_i2c_write_func(focal_write);
    init_i2c_read_func(focal_read);
