...
//static int iRawDataAvr[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
...
//static int m_NoiseDataTemp[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
...
FTS_TEST_INFO("%s: fts test end   bTestResult = %d \n",__func__,bTestResult);
...
//   int ibiggerValue = 0;
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        ReCode = WriteReg( REG_ScWorkMode, 0 );//ScWorkMode:  1㏒owaterproof 0:Non-waterproof
        if ( ReCode != ERROR_CODE_OK )
        {
            FTS_TEST_ERROR("Get REG_ScWorkMode Failed!");
            goto TEST_ERR;
