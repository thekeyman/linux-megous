...
#define BATTERY_LEVEL_1		(300)
#define PERCENT_LEVEL_1		(996)
#define BATTERY_LEVEL_2		(500)
#define PERCENT_LEVEL_2		(972)
#define BATTERY_LEVEL_3		(800)
#define PERCENT_LEVEL_3		(850)
#define PERCENT_BASE		(1000)
#define BASP_CYCLE_MAX		(7300)   /*the max cycle is 20 years*/
static void set_learned_cc_by_cycle(struct fg_chip *chip ,int cycle_count)
{
    if (cycle_count > BATTERY_LEVEL_1 && cycle_count <= BATTERY_LEVEL_2)
    {
        chip->learning_data.learned_cc_uah = div64_s64(((int64_t)chip->nom_cap_uah * PERCENT_LEVEL_1), PERCENT_BASE);
    }
    else if (cycle_count > BATTERY_LEVEL_2 && cycle_count <= BATTERY_LEVEL_3)
    {
        chip->learning_data.learned_cc_uah = div64_s64(((int64_t)chip->nom_cap_uah * PERCENT_LEVEL_2), PERCENT_BASE);
    }
    else if (cycle_count > BATTERY_LEVEL_3 && cycle_count <= BASP_CYCLE_MAX)
    {
        chip->learning_data.learned_cc_uah = div64_s64(((int64_t)chip->nom_cap_uah * PERCENT_LEVEL_3), PERCENT_BASE);
    }
    else
    {
        chip->learning_data.learned_cc_uah = (int64_t)chip->nom_cap_uah;
...
static void fg_cap_learning_post_process(struct fg_chip *chip)
{
	...
	cycle_count = fg_get_chg_cycle_count(chip);
	if (chip->clear_self_learn)
	{
		pr_info("chip->learning_data.learned_cc_uah = %ld, cycle_count = %d\n",(long)chip->learning_data.learned_cc_uah, cycle_count);
		set_learned_cc_by_cycle(chip, cycle_count);
	}
...
static int fg_of_init(struct fg_chip *chip)
{
	...
	chip->clear_self_learn = of_property_read_bool(chip->spmi->dev.of_node,"clear_self_learn");
	pr_info("clear the self learn %d", chip->clear_self_learn);
